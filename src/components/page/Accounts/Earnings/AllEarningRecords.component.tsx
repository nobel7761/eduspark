...
import AddEarningDialog from "@/components/Dialogs/AddEarningDialog";
...
const AllEarningRecordsComponent = () => {
  const { searchQuery } = useSearch();
  const [records, setRecords] = useState<EarningRecord[]>([]);
  const [earningsSummary, setEarningsSummary] = useState<{
    totalEarning: number;
    earningsByType: { [key: string]: number };
  }>({ totalEarning: 0, earningsByType: {} });
    ...
    const fetchRecords = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_BASE}/earnings/monthly?month=${monthStr}&year=${selectedYear}`
        );
        ...
        const data: EarningResponse = await response.json();
  ...
  const columnHelper = createColumnHelper<EarningRecord>();
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor((row) => row.studentId.studentId, {
        id: "studentId",
        header: "Student ID",
        cell: (info) => info.getValue(),
      }),
      columnHelper.accessor((row) => row.studentId.name, {
        id: "name",
        header: "Name",
        cell: (info) => info.getValue(),
      }),
      columnHelper.accessor((row) => row.studentId.class, {
        id: "class",
        header: "Class",
        cell: (info) => info.getValue(),
      }),
    ...
    <div className="p-4 rounded-md bg-gray-900 text-white">
      <div className="flex justify-between items-center mb-4">
        <div className="space-y-2">
          <h1 className="text-lg font-semibold">Earning Details</h1>
        </div>
...
export default AllEarningRecordsComponent;
